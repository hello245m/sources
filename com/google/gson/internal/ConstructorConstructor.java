package com.google.gson.internal;

import com.google.gson.InstanceCreator;
import com.google.gson.reflect.TypeToken;
import java.lang.reflect.Constructor;
import java.lang.reflect.Type;
import java.util.Map;

/* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
public final class ConstructorConstructor {
    private final Map<Type, InstanceCreator<?>> instanceCreators;

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$1, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass1<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;
        public final /* synthetic */ Type val$type;
        public final /* synthetic */ InstanceCreator val$typeCreator;

        public AnonymousClass1(ConstructorConstructor constructorConstructor, InstanceCreator instanceCreator, Type type) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$10, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass10<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;

        public AnonymousClass10(ConstructorConstructor constructorConstructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$11, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass11<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;

        public AnonymousClass11(ConstructorConstructor constructorConstructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$12, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass12<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;

        public AnonymousClass12(ConstructorConstructor constructorConstructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$13, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass13<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;

        public AnonymousClass13(ConstructorConstructor constructorConstructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$14, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass14<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;
        private final UnsafeAllocator unsafeAllocator;
        public final /* synthetic */ Class val$rawType;
        public final /* synthetic */ Type val$type;

        public AnonymousClass14(ConstructorConstructor constructorConstructor, Class cls, Type type) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$2, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass2<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;
        public final /* synthetic */ InstanceCreator val$rawTypeCreator;
        public final /* synthetic */ Type val$type;

        public AnonymousClass2(ConstructorConstructor constructorConstructor, InstanceCreator instanceCreator, Type type) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$3, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass3<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;
        public final /* synthetic */ Constructor val$constructor;

        public AnonymousClass3(ConstructorConstructor constructorConstructor, Constructor constructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$4, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass4<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;

        public AnonymousClass4(ConstructorConstructor constructorConstructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$5, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass5<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;
        public final /* synthetic */ Type val$type;

        public AnonymousClass5(ConstructorConstructor constructorConstructor, Type type) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$6, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass6<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;

        public AnonymousClass6(ConstructorConstructor constructorConstructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$7, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass7<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;

        public AnonymousClass7(ConstructorConstructor constructorConstructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$8, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass8<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;

        public AnonymousClass8(ConstructorConstructor constructorConstructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    /* JADX INFO: Add missing generic type declarations: [T] */
    /* renamed from: com.google.gson.internal.ConstructorConstructor$9, reason: invalid class name */
    /* loaded from: C:\Users\elonm\Documents\fdtool\gzhdex\00O000ll111l_0.dex */
    public class AnonymousClass9<T> implements ObjectConstructor<T> {
        public final /* synthetic */ ConstructorConstructor this$0;

        public AnonymousClass9(ConstructorConstructor constructorConstructor) {
        }

        @Override // com.google.gson.internal.ObjectConstructor
        public T construct() {
        }
    }

    public ConstructorConstructor(Map<Type, InstanceCreator<?>> map) {
    }

    private <T> ObjectConstructor<T> newDefaultConstructor(Class<? super T> cls) {
    }

    private <T> ObjectConstructor<T> newDefaultImplementationConstructor(Type type, Class<? super T> cls) {
    }

    private <T> ObjectConstructor<T> newUnsafeAllocator(Type type, Class<? super T> cls) {
    }

    public <T> ObjectConstructor<T> get(TypeToken<T> typeToken) {
    }

    public String toString() {
    }
}
